...
const StudentDashboard = () => {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 font-black text-blue-600 animate-pulse text-2xl uppercase tracking-tighter">Synchronizing Portal...</div>;
    ...
    const MobileTabButton = ({ id, icon: Icon, label }) => (
        <button
            onClick={() => {
                setActiveTab(id);
                navigate(`/student?tab=${id}`, { replace: true });
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans select-none">
            ...
            <div className="flex-1 flex flex-col md:flex-row relative">
                {/* Desktop Sidebar */}
                <aside className="hidden md:block md:w-64 bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-6 md:fixed md:top-[160px] md:bottom-8 md:left-8 z-20">
                    <nav className="space-y-1">
                        <SidebarLink id="dashboard" icon={LayoutDashboard} label="Dashboard" />
                        <SidebarLink id="exams" icon={BookOpen} label="Exams" />
                        <SidebarLink id="quizzes" icon={Rocket} label="Quizzes" />
                        <SidebarLink id="quiz-results" icon={Award} label="Quiz Results" />
                        <SidebarLink id="results" icon={Award} label="Results" />

                        <SidebarLink id="events" icon={Calendar} label="Events" />
                        <SidebarLink id="notices" icon={Bell} label="Notice Board" />
                ...
                <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white border-t border-slate-100 pt-1 pb-1 px-2 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] safe-pb">
                    <div className="h-16 bg-white border border-slate-200/60 rounded-[1.5rem] shadow-sm flex items-center justify-around px-2">
                        <MobileTabButton id="dashboard" icon={LayoutDashboard} label="Home" />
                        <MobileTabButton id="exams" icon={BookOpen} label="Exams" />
                        <MobileTabButton id="quizzes" icon={Rocket} label="Quizzes" />
                        <MobileTabButton id="quiz-results" icon={Award} label="Quiz" />
                        <MobileTabButton id="results" icon={FileText} label="Results" />
                        <MobileTabButton id="events" icon={Calendar} label="Events" />
                        <MobileTabButton id="notices" icon={Bell} label="Notice" />
                                        ...
                                        {notices.length > 0 ? notices.map((notice, idx) => {
                                            ...
                                            return (
                                                <motion.div 
                                                    ...
                                                    className={`rounded-[2rem] border transition-all cursor-pointer overflow-hidden ${
                                                        isExpanded ? 'bg-white shadow-xl border-blue-200' : 'bg-white hover:bg-slate-50 border-slate-100 shadow-sm'
                                                    }`}
                                                    ...
                                                    <div className="p-5 flex justify-between items-center">
                                                        <div className="flex items-center gap-4">
                                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                                                                notice.type === 'urgent' ? 'bg-rose-50 text-rose-500' :
                                                            ...
                                                            <div>
                                                                <h3 className="font-black text-slate-800 text-base flex items-center gap-2">
                                                                    ...
                                                                    <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
                                                                        notice.type === 'urgent' ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-600'
                                                                    }`}>
                                                                        {notice.type}
                                                                    </span>
                                                                </h3>
                ...
                {viewingResource && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 sm:p-12">
                        ...
                        <motion.div 
                            ...
                            className={`relative bg-white shadow-2xl overflow-hidden flex flex-col transition-all duration-500 ${
                    ...
                    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 md:p-8">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setViewingQuizResult(null)} className="absolute inset-0 bg-slate-900/80 backdrop-blur-xl" />
                            <motion.div 
                                ...
                                className="relative bg-white w-full max-w-2xl max-h-[90vh] rounded-[2rem] sm:rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col"
                            >
                            {/* Gradient Header */}
                                                ...
                                                <div key={idx} className="bg-white rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-6 border border-slate-100 shadow-sm transition-all group overflow-hidden relative">
                                                    ...
                                                    <div className="flex justify-between items-start gap-4 mb-4">
                                                        ...
                                                        <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border shrink-0 ${
                                                            ans ? (isCorrect ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-500 border-rose-100') 
